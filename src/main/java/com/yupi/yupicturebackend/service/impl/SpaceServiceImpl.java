...
public class SpaceServiceImpl extends ServiceImpl<SpaceMapper, Space> implements SpaceService {
    ...
    public LambdaQueryWrapper<Space> getQueryWrapper(SpaceQueryRequest spaceQueryRequest) {
        ...
        queryWrapper.eq(ObjectUtil.isNotNull(id), Space::getId, id)
                .eq(ObjectUtil.isNotNull(userId), Space::getUserId, userId)
                .like(StrUtil.isNotBlank(spaceName), Space::getSpaceName, spaceName)
                .eq(ObjectUtil.isNotNull(spaceLevel), Space::getSpaceLevel, spaceLevel)
                // SQL 语句拼接
                .apply(ObjectUtil.isNotNull(sortField), "order by %s %s", sortField, sortOrder);
    ...
    @Override
    public Space getSpace(SpaceVO spaceVO) {
        if (spaceVO == null) {
            return null;
        }
        // 封装空间
